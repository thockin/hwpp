...
TEST(test_add_and_lookup) {
	...
	hwpp::language::Variable *v;

	s.add("x", hwpp::language::Variable(true));
	TEST_ASSERT(s.context_size() == 1);
	v = s.lookup("x");
	TEST_ASSERT(v->type() == hwpp::language::Variable::TYPE_BOOL);
	TEST_ASSERT(v->bool_value() == true);

	s.add("y", hwpp::language::Variable(hwpp::Value(0)));
	TEST_ASSERT(s.context_size() == 2);
	v = s.lookup("y");
	TEST_ASSERT(v->type() == hwpp::language::Variable::TYPE_INT);
	TEST_ASSERT(v->int_value() == 0);

	s.add("z", hwpp::language::Variable(string("")));
	TEST_ASSERT(s.context_size() == 3);
	v = s.lookup("z");
	TEST_ASSERT(v->type() == hwpp::language::Variable::TYPE_STRING);
	...
	s.add("a", hwpp::language::Variable(hwpp::Value(-1)));
	TEST_ASSERT(s.context_size() == 1);
	v = s.lookup("a");
	TEST_ASSERT(v->type() == hwpp::language::Variable::TYPE_INT);
	...
	s.add("x", hwpp::language::Variable(hwpp::Value(93)));
	TEST_ASSERT(s.context_size() == 2);
	v = s.lookup("x");
	TEST_ASSERT(v->type() == hwpp::language::Variable::TYPE_INT);
